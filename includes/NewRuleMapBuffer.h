#ifndef NEW_RULE_MAP_BUFFER_H
#define NEW_RULE_MAP_BUFFER_H
...
namespace procgui
{
    ...
    // manage:
    //   - synchronization between 'NewRuleMapBuffer':
    ...
    // Target or others 'NewRuleMapBuffer'.
    // 
    //   - duplication:
    ...
    // rule and its replacement are in different 'NewRuleMapBuffer'.
    // 
    ...
    class NewRuleMapBuffer : public Observer<Target>
    {
    public:
        /* must be a derived class of RuleMap */
        static_assert(std::is_base_of<RuleMap<typename Target::successor>, Target>::value, "NewRuleMapBuffer must refer to a derived class of RuleMap");
        ...
        // Constructor
        explicit NewRuleMapBuffer(std::shared_ptr<Target> target_);

        // The copy constructor and the assignment operator are needed because
        // we need to register a callback for the new object to
        // 'Observer<Target>'
        NewRuleMapBuffer(const NewRuleMapBuffer& other);
        NewRuleMapBuffer(const NewRuleMapBuffer& other, std::shared_ptr<Target> target);
        NewRuleMapBuffer(NewRuleMapBuffer&& other);
        NewRuleMapBuffer& operator=(const NewRuleMapBuffer& other);
        NewRuleMapBuffer& operator=(NewRuleMapBuffer&& other);
