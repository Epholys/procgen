...
{
    auto it = std::find_if(begin(all_orders_json_name),
                           end(all_orders_json_name),
    ...
    if (it == end(all_orders_json_name))
    {
        controller::LoadMenu::add_loading_error_message(
            "One InterpretationMap's order does not exist, it is now set to go_forward");
    }
    else
    {
...
class InterpretationMap : public RuleMap<Order>
{
  ...
  private:
    ...
    void load(Archive& ar, const u32)
    {
        ...
        {
            controller::LoadMenu::add_loading_error_message(
                "One or more InterpretationMap's key was too big, it is now cropped.");
        }
        if (void_key)
        {
            controller::LoadMenu::add_loading_error_message(
                "One or more InterpretationMap's key was empty, so it was ignored.");
        }
    }
};
...
const InterpretationMap default_interpretation_map {{'F', go_forward},
                                                    {'-', turn_left},
                                                    {'+', turn_right},
                                                    {'[', save_position},
                                                    {']', load_position}};
