...
void WindowController::right_click_menu(sf::RenderWindow& window,
                                        std::list<procgui::LSystemView>& lsys_views)
{
    if (ImGui::BeginPopupContextVoid())
    {
        ...
        {
            lsys_views.emplace_front(
                ext::sf::Vector2d(real_mouse_position(sf::Mouse::getPosition(window))),
                                    ...
                                    std::list<procgui::LSystemView>& lsys_views)
{
    ...
    for (const auto& event : events)
    {
        ...
        if (event.type == sf::Event::Closed
            || (!imgui_io.WantCaptureKeyboard && event.type == sf::Event::KeyPressed
                && event.key.code == sf::Keyboard::Escape))
        {
            // Check if a LSystemView is saved but not modified
            quit_popup_open_ = std::any_of(begin(lsys_views),
                                           end(lsys_views),
            ...
            else
            {
                procgui::PopupGUI quit_popup = {
                    ...
                    []() {
                        ImGui::Text(
                            "At least one L-System is not saved.\nDo you still want to quit?");
                    },
                    false,
                    "Yes, quit",
                    "No, stay",
                     ...
                     || sf::Keyboard::isKeyPressed(sf::Keyboard::RControl)))
        {
            ...
            {
                load_menu_open_ = true;
                load_position_ = ext::sf::Vector2d(
                    real_mouse_position({int(sfml_window::window.getSize().x / 2),
                                         int(sfml_window::window.getSize().y / 2)}));
            }
        }
        ...
        else if (load_menu_open_ && event.type == sf::Event::TextEntered)
        {
        ...
        {
            if (!imgui_io.WantCaptureMouse && event.type == sf::Event::MouseWheelMoved)
            {
    ...
    {
        load_menu_open_ =
            !load_menu_.open(lsys_views, load_position_, key_to_menus, unicode_to_load_window);
    }
    ...
    // Dragging behaviour
    if (has_focus_ && view_can_move_ && !imgui_io.WantCaptureMouse
