...
void LinearGradient::set_keys(const keys& keys)
{
    ...
    // Sort the elements.
    std::sort(begin(keys_), end(keys_), [](const auto& p1, const auto& p2) {
...
sf::Color LinearGradient::get(float f)
{
    ...
    auto superior_it = std::find_if(begin(keys_), end(keys_), [f](const auto& p) {
        return f <= p.position;
    });
#ifdef DEBUG_CHECKS
    ...
    auto inferior_index = superior_index == 0 ? 0
                                              : superior_index - 1; // ...and the lower-bound one.
...

{
    ...
    Expects(std::is_sorted(begin(keys), end(keys), [](const auto& a, const auto& b) {
...
void DiscreteGradient::set_keys(const keys& keys)
{
    ...
    Ensures(std::is_sorted(begin(keys_), end(keys_), [](const auto& a, const auto& b) {
        return a.index < b.index;
    }));
...
void DiscreteGradient::generate_colors()
{
    ...
    {
        // Interpolate
        float factor = (static_cast<float>(i) - inferior->index)
                       / (superior->index - inferior->index);
        ImVec4 imcolor;
        imcolor.x = inferior->imcolor.x * (1 - factor) + superior->imcolor.x * factor;
        imcolor.y = inferior->imcolor.y * (1 - factor) + superior->imcolor.y * factor;
        imcolor.z = inferior->imcolor.z * (1 - factor) + superior->imcolor.z * factor;
