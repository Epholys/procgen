...
std::vector<sf::FloatRect> sub_boxes(const std::vector<sf::Vertex>& vertices, int max_boxes)
{
    Expects(max_boxes > 0);
...
std::pair<sf::Vector2f, sf::Vector2f>
    intersection_with_bounding_box(const Line& line, const sf::FloatRect& bounding_box)
{
    ...
    possible_opposite_intersection.first = intersection(line,
                                                        possible_opposite_intersection_line.first);
    possible_opposite_intersection.second = intersection(
        line,
        possible_opposite_intersection_line.second);

    // Select the closest intersection.
    sf::Vector2f intersection {line.point};
    if (distance(line.point, possible_intersection.first)
        <= distance(line.point, possible_intersection.second))
    {
    ...
    if (distance(line.point, possible_opposite_intersection.first)
        <= distance(line.point, possible_opposite_intersection.second))
    {
