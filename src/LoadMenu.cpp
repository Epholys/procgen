        ...
        catch (const cereal::RapidJSONException& e)
        {
            ...
            procgui::PopupGUI format_popup =
                { "Error##FORMAT",
                  [this]()
                  {
                      std::string message = "Error: file '" + array_to_string(file_to_load_) + "' isn't a valid or complete JSON L-System file.";
            ...
            procgui::PopupGUI warning_popup =
                { "Warning##ISSUE",
                  [this]()
                  {
                      ...
                      {
                          message = "Warning: file '" + array_to_string(file_to_load_) + "' has some issues:\n";
                      }
                      else
                      {
                          message = "Warning: file '" + array_to_string(file_to_load_) + "' has one issue:\n";
                      }
                      ImGui::Text(message.c_str());
            ...
            if(!ifs.is_open())
            {
                procgui::PopupGUI file_error_popup =
                    { "Error##PERM",
                      ...
                      {
                          // TODO true name
                          std::string error_message = "Error: can't open file: "+array_to_string(file_to_load_);
                                   ...
                                   column_layout layout)
    {
        ...
        if (!files.empty())
        {
            if (key == sf::Keyboard::Key::Right)
            {
                ...
                else if (file_idx_ >= (int)files.size()) // We are below the last element in the last column
                {
            ...
            else if (key == sf::Keyboard::Key::Down)
            {
                ...
                if (file_idx_ > file_per_column * (n_column-1)) // File is in last column
                {
                    ...
                    {
                        file_idx_ = file_per_column * (n_column - 1); // Go to the top of the last column
                    }
                }
